...
function createTextTexture(
  ...
  // `font` is a full CSS font shorthand (e.g. "700 30px sans-serif"), so pull
  // the pixel size out explicitly instead of parseInt-ing the whole string
  // (which would grab the font-weight and wildly distort the texture aspect).
  const fontSizeMatch = font.match(/(\d+(?:\.\d+)?)px/);
  const fontSizePx = fontSizeMatch ? parseFloat(fontSizeMatch[1]) : 30;
